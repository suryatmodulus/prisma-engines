...
pub(crate) fn calculate_steps(schemas: Pair<&SqlSchema>, flavour: &dyn SqlFlavour) -> Vec<SqlMigrationStep> {
    ...
    flavour
        ...
        .chain(create_tables(&db).map(SqlMigrationStep::CreateTable))
...
// please later.
fn drop_tables(db: &DifferDatabase<'_>) -> (Vec<DropTable>, Vec<DropForeignKey>) {
    let (dropped_tables_count, dropped_fks_count) = db.dropped_tables().fold((0, 0), |(tables, fks), item| {
...
fn add_foreign_keys(db: &DifferDatabase<'_>) -> Vec<AddForeignKey> {
    ...
    if !db.flavour.can_migrate_arity_change_on_fk_columns() {
        todo!()
    }
...
fn push_previous_usages_as_defaults_in_altered_enums(db: &DifferDatabase<'_>, alter_enums: &mut [AlterEnum]) {
